...
func TestCommandAcceptsOnlyOneInvocationArgument(t *testing.T) {
	for _, arguments := range [][]string{
		...
		if stdout.Len() != 0 ||
			stderr.String() != "raw OCI driver failed: invalid arguments\n" {
			t.Fatalf("argument output changed: %q %q", stdout.String(), stderr.String())
		}
	}
}
...
func TestCommandFailureIsFixedAndDoesNotEchoPath(t *testing.T) {
	...
	if code != 1 || stdout.Len() != 0 ||
		stderr.String() != "raw OCI driver failed: execution unavailable\n" ||
