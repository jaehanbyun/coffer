...
action="${1:-}"
domain_name="coffer-kolla-aio-stage4"
...
management_mac="52:54:00:cf:fe:03"
external_mac="52:54:00:cf:fe:04"
...
create() {
    ...
    if "${virsh_command[@]}" net-dumpxml default |
        grep -qE "${ip_address}|${management_mac}|${external_mac}"; then
        echo "planned Stage 4 address or MAC is already reserved" >&2
        exit 73
    fi
