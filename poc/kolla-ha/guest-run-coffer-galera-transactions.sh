...
marker_root="${state_root}/coffer-galera-transactions"
owner_marker="${marker_root}/owner"
complete_marker="${marker_root}/complete"
owner_value="coffer-stage5-galera-transactions-v1"
complete_value="concurrency=one-winner retry=1205-attempt-2 residue=0"
temporary_helper="/run/coffer-stage5-galera-transactions.py"
...
case "${action}" in
    status)
        printf 'coffer_galera_transactions_marker state=%s mutation=none\n' \
            "${state}"
        ;;
    start)
        if test "${state}" = completed; then
            printf 'coffer_galera_transactions_marker state=completed idempotent=yes\n'
            exit 0
        fi
        ...
        require_owner
        printf 'coffer_galera_transactions_marker state=running owner=verified\n'
        ;;
    complete)
        test "${state}" = running
        require_owner
        write_marker "${complete_marker}" "${complete_value}"
        printf 'coffer_galera_transactions_marker state=completed result=passed\n'
        ;;
esac
