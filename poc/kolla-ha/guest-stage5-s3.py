...
EXTRA_BUCKET = "coffer-stage5-extra-denied"
...
def sentinel_payload() -> bytes:
    seed = b"coffer-stage5-rgw-ha-sentinel\n"
    return (seed * ((PAYLOAD_SIZE // len(seed)) + 1))[:PAYLOAD_SIZE]


def ensure_sentinel(s3: Any) -> str:
    payload = sentinel_payload()
    digest = hashlib.sha256(payload).hexdigest()
    try:
        metadata = s3.head_object(Bucket=REGISTRY_BUCKET, Key=SENTINEL_KEY)
        if int(metadata["ContentLength"]) != len(payload):
            raise RuntimeError("existing sentinel has an unexpected size")
        if metadata.get("Metadata", {}).get("sha256") != digest:
            raise RuntimeError("existing sentinel has unexpected metadata")
    except ClientError as error:
        if status_code(error) != 404:
            raise
        s3.put_object(
    ...
    downloaded = s3.get_object(Bucket=REGISTRY_BUCKET, Key=SENTINEL_KEY)[
        "Body"
    ].read()
    if hashlib.sha256(downloaded).hexdigest() != digest:
        raise RuntimeError("private sentinel changed during round trip")
    return digest
...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: guest-stage5-s3.py REGISTRY_USER_JSON DENIAL_USER_JSON"
        )
    ...
    registry_buckets = {
        entry["Name"] for entry in registry.list_buckets()["Buckets"]
    }
    denial_buckets = {
        entry["Name"] for entry in denial.list_buckets()["Buckets"]
    }
    ...
    evidence = {
        ...
        "sentinel_bytes": PAYLOAD_SIZE,
        "sentinel_sha256": ensure_sentinel(registry),
    }
