...
if [[ "${action}" != "preflight" || "$#" -ne 2 ]]; then
    echo "usage: $0 preflight <ssh-target>" >&2
    exit 64
fi
...
uv run python - "${topology}" "${work}/host-inventory.json" <<'PYTHON'
...
if pool is not None:
    require(pool["state"] == "running", "required libvirt pool is not running")
...
for name in domain_names:
    require(name not in existing_domain_names, f"domain already exists: {name}")
...
for name in planned_volume_names:
    require(name not in existing_volume_names, f"volume already exists: {name}")
...
for name in planned_network_names:
    require(name not in existing_network_names, f"network already exists: {name}")
for bridge in planned_bridges:
    require(bridge not in existing_bridges, f"bridge already exists: {bridge}")
...
for line in libvirt["default_network_leases"]:
    for match in re.findall(r"\b(?:\d{1,3}\.){3}\d{1,3}/\d{1,2}\b", line):
...
for network_name, network in networks.items():
    ...
    for key, value in network.items():
        if (
            ...
            require(address in subnet, f"{network_name}.{key} is outside its subnet")
...
for mac in planned_macs:
    ...
    require(mac not in existing_macs, f"planned MAC is already in use: {mac}")
...
result: dict[str, Any] = {
    ...
    "memory_available_after_budget_gib": round(
        available_memory_gib - budget["memory_gib"], 1
    ),
    "filesystem_available_after_budget_gib": round(
        filesystem_available_gib - budget["total_logical_disk_gib"], 1
    ),
    "pool_available_after_budget_gib": round(
        pool_available_gib - budget["total_logical_disk_gib"], 1
    ),
}
