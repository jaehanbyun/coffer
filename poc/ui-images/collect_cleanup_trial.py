...
SCHEMA = "coffer.ui-os-cleanup-evidence/v1"
...
IMAGE_NAME = re.compile(
    r"^localhost/coffer-ui-trial-(horizon|skyline)-(before|after):"
    r"2026\.1-os-cleanup$"
)
...
def container_json(
    *,
    ...
    if process.returncode != 0 or process.stderr:
        raise CollectionError("container evidence collection failed")
    try:
...
def collect(
    *,
    ...
    runtime_collector = Path(__file__).with_name("collect_runtime.py")
    inventories = {
        key: container_json(
            image=images[key],
...
def main() -> int:
    ...
    try:
        ...
        print("coffer-ui-os-cleanup-collector: invalid evidence")
        return 2
    return 0
