...
external VIP `192.168.122.221`, with 8 vCPUs, 40 GiB RAM, a 220-GiB sparse
root in only the `coffer-rgw` pool, and libvirt autostart disabled. The guest
reports 41 running containers, zero non-running or unhealthy containers, both
VIPs present, and owner-only identity and acceptance evidence at mode 0600.
Horizon and Skyline both show `preview-proof`, quota, and the same repository
detail. OCI digest
...
The third independent native ARM64 derivative changes only `urllib3` 2.6.3
to 2.7.0 using the official non-yanked PyPI wheel. Both surfaces retain the
accepted cleanup OS inventory and all non-target Python distribution version
multisets. `pip check`, no-network HTTPS pool construction, official source
...
pass the existing secret and absolute remaining-count gates. Equal-set,
empty-set, malformed-contract, and unexpected-removal fixtures pass. This
does not waive any union finding or permit either scanner to be omitted.

The sixth independent native ARM64 derivative changes only `click` 8.3.1 to
8.3.3 using the official non-yanked PyPI wheel. Both surfaces preserve the
accepted cleanup OS inventory and every non-target Python distribution version
multiset. `pip check`, an offline real-command `CliRunner` invocation, official
source hashes, package-local bytecode boundaries, Coffer UI runtime hashes,
lineage, and build-input absence pass.

Trivy remains unchanged at Horizon 31 and Skyline 16 High. Docker Scout removes
exactly `CVE-2026-7246`, changing Horizon 34 to 33 and Skyline 19 to 18 High.
...
- Completed the local Horizon Project Registry dashboard. The out-of-tree
  wheel contains catalog-gated panel/group enabled files, repository policy
  mirror, list/quota/create/detail server-rendered views, and all templates.
  It exposes no destructive or private backend operation.
- `make -C ui/horizon verify` passes the exact Kolla/OpenStack 2026.1 source
  and dependency verifier, 36 adapter/Django flow tests, and compilation.
