...
def rgw_socket_payload() -> bytes:
    return (
        "\n".join(
            "ceph_daemon_socket_up"
            f'{{ceph_daemon="{daemon}",hostname="{host}"}} 1'
...
def populate_responses(
    ...
    for surface, payload in payloads.items():
        endpoint = target["sources"][surface]["evidence_urls"]["before"]
        responses[response_path(endpoint["url"])] = (
            "application/json",
            evidence(surface, payload),
        )
    ...
    for endpoint in target["sources"]["galera"]["instances"].values():
        responses[response_path(endpoint["url"])] = (
            "text/plain; version=0.0.4",
            galera_payload(),
        )
    ...
    responses[response_path(rgw["ingress"]["url"])] = (
        "text/plain; version=0.0.4",
        haproxy_payload({"rgw-ingress": STORAGE[:2]}),
    )
