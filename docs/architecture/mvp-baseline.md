...
3. Unified repository/quota Alembic schema, legacy repository-row adoption, project quota admission, PostgreSQL/MariaDB row locks, multi-worker claims/fencing, process abandonment, exact-digest reconciliation, the runnable one-shot/periodic process, non-mutating tagged/digest-only filesystem inventory, atomic empty-ledger import/rollback, exact read-only post-import ledger comparison, and an injected-authentication live-presence core have bounded local evidence. Deterministic unique-descriptor SQLite characterization is approximately linear through 5,000 manifests but is not a production capacity result. Production still needs exact-release RGW inventory qualification, a selected maintenance identity and secret-delivery path, an accepted workload target plus representative shared-SQL/private-TLS comparison, writer exclusion, rollout/backup/rollback procedures, authenticated TLS reconciliation in the integrated RGW deployment, packaging/clock/deadlock/Galera and metric-aggregation policy, replica-level failure tests, and non-bypassable ingress.
