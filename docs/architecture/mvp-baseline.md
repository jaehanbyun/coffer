...
Validates secret-free evidence from two write-stopped scans performed through the exact pinned Distribution repository/manifest storage enumerators. It fails closed on page, snapshot, authority, digest, media, size, duplicate, and nested-index inconsistencies, then emits deterministic immutable project/repository/content facts without opening Distribution, object storage, or SQL. The current filesystem helper is disposable PoC evidence only; RGW configuration/credentials, production maintenance, and admission cutover remain separate approval gates under proposed ADR 0011.

### Coffer inventory baseline importer

Consumes one canonical `coffer.inventory/v1` plus an independently supplied SHA-256, verifies every redundant graph and aggregate before database access, and imports only into an empty migrated ledger with existing exact repository/quota authority. Revision `0004` stores one immutable baseline marker; marker, deterministic committed reservations, descriptor reference counts, manifests, and honest usage values commit in one transaction. Exact replay is a no-op, a different baseline or partial ledger fails closed, and known database deadlock/serialization failures have a bounded whole-transaction retry. The current SQLite/PostgreSQL/MariaDB evidence is disposable only; production writer exclusion, backup/restore, scale, comparison, rollback, and admission enablement remain operator gates under proposed ADR 0012.
...
Stores resource identity, stable project mapping, repository policy, signing-key metadata, logical quota reservations/usage observations, monotonic reconciliation versions, expiring worker claims, the singleton inventory-import marker, operation records, and audit references. One Alembic revision chain owns repository and quota tables; normal API, token, admission, import, and reconciliation startup validates the exact revision and never creates production tables implicitly. Revision `0003` can strictly adopt the exact legacy repository table without rewriting rows; revision `0004` adds the marker but does not inspect OCI payloads or perform an import. The database does not become a permanent registry credential store. Blob content and the canonical manifest payload remain in object storage.
...
- Run the online Alembic migration as a separate owner-controlled job before application rollout. An exact legacy repository table can be adopted; drift, offline conditional migration, and application-side auto-upgrade fail closed. Local rehearsals cover write-stopped tagged/digest-only inventory plus atomic empty-ledger import/rollback on SQLite, PostgreSQL, and MariaDB. Production still requires backup/restore, exact RGW/helper qualification, representative-scale transaction evidence, authenticated comparison, rollback, and cutover ownership.
...
| Persistence | SQLAlchemy/Alembic through `oslo.db`; MariaDB/Galera as the operator baseline, PostgreSQL supported by the PoC schema | Aligns with common OpenStack control-plane deployments; revisions `0001`–`0004`, row locks, claims, abandoned-process recovery, and atomic baseline import pass both disposable engines |
...
3. Unified repository/quota Alembic schema, legacy repository-row adoption, project quota admission, PostgreSQL/MariaDB row locks, multi-worker claims/fencing, process abandonment, exact-digest reconciliation, the runnable one-shot/periodic process, non-mutating tagged/digest-only filesystem inventory, and atomic empty-ledger import/rollback have bounded local evidence. Production still needs exact-release RGW inventory qualification, representative-scale import plus authenticated post-import comparison, rollout/backup/rollback procedures, authenticated TLS reconciliation in the integrated RGW deployment, packaging/clock/deadlock/Galera and metric-aggregation policy, replica-level failure tests, and non-bypassable ingress.
...
- `docs/adrs/0012-import-existing-content-into-empty-quota-ledger.md` proposes the singleton-marker, one-transaction baseline import and its remaining production gates.
