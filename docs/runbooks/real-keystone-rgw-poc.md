...
ADR 0009 quota admission is validated separately under `poc/quota/`: Distribution remains private, Docker/Podman/Skopeo publish through the edge, concurrent manifest admission returns one 201 and one 429, missing quota returns 503, and physical staging is measured separately from logical usage. `poc/quota-sql/` proves all three Alembic revisions, exact legacy repository-row adoption and downgrade retention, independent-connection row locks, multi-worker claims, spawned-process abandonment, expiry recovery, and token fencing on PostgreSQL and MariaDB; `poc/quota-reconciliation/` proves exact-digest repair against isolated unmodified Distribution; and the installed runner proves one-shot exact-404 recovery plus deterministic periodic lifecycle locally. These fixtures do not inventory existing OCI content or close production credential-helper, rollout/backups, authenticated TLS reconciliation in the integrated RGW topology, packaging/Galera/metric-aggregation policy, or separate-host ingress gates. See `docs/runbooks/quota-schema-reconciliation.md` for that authority boundary.
